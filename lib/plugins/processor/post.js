...
module.exports = ctx => {
  function processPost(file) {
    ...
    return Promise.all([
      ...
      data.date = common.parseDate(data.date, timezone);
      if (!data.date &&
          info && info.year && (info.month || info.i_month) && (info.day || info.i_day)) {
        data.date = new Date(
          info.year,
      ...
      data.updated = common.parseDate(data.updated, timezone);
